...
def show_board_in_window(board):
            ...
            for col in range(9):
                if entries[row][col].get() == "":
                    ...
                    for num in range(1, 10):
                        if is_valid([[int(e.get()) if e.get() else 0 for e in row] for row in entries], row, col, num):
                            options.append(num)
                    if len(options) < min_options and len(options) > 0:
                        min_options = len(options)
                        best_cell = (row, col)
                        best_num = options[0]
        if best_cell:
            row, col = best_cell
            messagebox.showinfo("Hinweis", f"Versuche die Zahl {best_num} in die Zelle ({row + 1}, {col + 1}) einzusetzen. Diese Zelle hat nur {min_options} mögliche Zahl(en).")
        else:
            messagebox.showinfo("Hinweis", "Es gibt derzeit keine offensichtlichen Vorschläge.")
    ...
    hint_button = tk.Button(root, text="Hinweis", command=hint_button_click, font=my_font, bg='#17a2b8', fg='#ffffff', activebackground='#138496', activeforeground='#ffffff')
    hint_button.pack(side=tk.BOTTOM, padx=10, pady=10)

    update_number_buttons()
    root.mainloop()

def main():
    # Zuerst nachfragen, ob ein gespeichertes Spiel geladen werden soll
    if messagebox.askyesno("Spiel laden", "Möchtest du ein gespeichertes Spiel laden?"):
        sudoku_board = load_game()
        if sudoku_board:
            show_board_in_window(sudoku_board)
            return

    # Falls kein Spiel geladen wird, Schwierigkeitsgrad auswählen
    root = tk.Tk()
